...
const CardLogo = ({ type }: { type: string }) => {
  switch (type) {
    case "visa":
      return (
        <div className="flex items-center justify-center w-10 h-5 bg-blue-600 rounded text-white text-xs font-bold">
          VISA
        </div>
      )
    case "mastercard":
    ...
    case "rupay":
      return (
        <div className="flex items-center justify-center w-10 h-5 bg-green-600 rounded text-white text-xs font-bold">
          RuPay
        </div>
      )
    case "discover":
...
export function PaymentDetailsTwo({
  ...
  return (
    <form
      ...
      className={cn(
        " max-w-2xl  w-full mx-auto space-y-6",
        className
      )}
    >
      ...
      <div className="flex flex-col border  rounded-lg gap-6 p-6 bg-card/50 shadow-md">
        ...
        <div>
          ...
          />
          {errors.nameOnCard && (
            <p className="text-red-500 text-sm">{errors.nameOnCard.message}</p>
          )}
        </div>
        ...
        <div>
          ...
          <div className="relative flex items-center">
            ...
            <Input
              ...
              maxLength={20} // 16 digits + 3 spaces (xxxx xxxx xxxx xxxx)
              inputMode="numeric"
              pattern="[0-9 ]*" // only digits and spaces
              {...register("cardNumber", {
                required: "Card number is required",
                validate: (val) => val ?
                  validateLuhn(val) : "Invalid card number"
              })}
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full">
            ...
            {errors.middleLastName && (
              <p className="text-red-500 text-sm">{errors.middleLastName.message}</p>
            )}
          </div>
        </div>
