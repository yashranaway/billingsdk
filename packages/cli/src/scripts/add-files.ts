...
export const addFiles = async (
  ...
  const base =
    ...
    process.env.BILLINGSDK_REGISTRY_BASE ||
    `file://${path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../public/tr")}`;
  ...
  if (u.protocol === "file:") {
    ...
    if (!fs.existsSync(filePath)) {
      throw new Error(
        `Transport not found at ${filePath}. Did you build templates?`,
      );
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          `Failed to fetch transport from ${u.toString()} (status ${res.status}).`,
        );
      }
  ...
  for (const file of result.files) {
    if (
      ...
      throw new Error(
        `Refusing to write outside project: invalid target '${file.target}'`,
      );
    }
    ...
    const relativePath =
      path.relative(cwd, resolved) || path.basename(resolved);
    ...
    try {
      ...
      if (!realDir.startsWith(realCwd + path.sep) && realDir !== realCwd) {
        throw new Error(
          `Resolved real path escapes project directory: '${realDir}'`,
        );
      }
      ...
      if (fs.existsSync(resolved)) {
        if (fileName === ".env.example") {
          ...
          if (options.verbose)
            console.log(`wrote ${path.relative(cwd, envPath) || ".env"}`);
        } else {
          const mode =
            options.onConflict ??
          ...
          if (mode === "overwrite") {
            ...
            if (isCancel(overwrite)) {
              throw new Error(
                "Setup cancelled by user during overwrite prompt",
              );
  ...
  if (failures.length) {
    throw new Error(
      `Aborting: failed to add ${failures.length} file(s): ${failures.join(", ")}`,
    );
  }
  if (result.dependencies && options.installDeps !== false && !options.dryRun) {
    ...
    if (res.error || (typeof res.status === "number" && res.status !== 0)) {
      ...
      throw (
        res.error || new Error(`Installer exited with status ${res.status}`)
      );
    }
