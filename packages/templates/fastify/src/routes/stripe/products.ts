...
export default async function productsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/", async (request, reply) => {
    try {
      const { limit, starting_after, active } = request.query as Record<
        string,
        any
      >;
      ...
      if (typeof limit === "string") {
        ...
        if (
          ...
          return reply
            .status(400)
            .send({ error: "limit must be an integer between 1 and 100" });
        }
  ...
  fastify.get("/prices", async (request, reply) => {
    try {
      const { product_id, limit, starting_after, active } =
