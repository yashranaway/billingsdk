...
The `gradientColors` prop accepts an array of color strings (preferably in rgba format with alpha transparency). The gradient creates a smooth animated background effect that moves from left to right.

### Features:
- **Theme-aware**: Automatically adapts text colors and filter effects for light/dark themes
- **Smooth animation**: 30-second linear animation that loops infinitely
- **Customizable**: Works with any number of colors (optimal with 4 colors)
- **Subtle transitions**: Gentle fade-in/out animations (0.15s duration)
