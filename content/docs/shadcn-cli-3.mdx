...
Add components directly using their registry URLs:
...
<Tabs items={['npx', 'pnpm', 'yarn', 'bun']} defaultValue="npx" groupId="installation-tabs">
  ...
  <Tab value="yarn">
    ```bash
    yarn dlx shadcn@latest add https://billingsdk.com/r/<component-name>.json
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bunx shadcn@latest add https://billingsdk.com/r/<component-name>.json
    ```
  </Tab>
</Tabs>
...
```bash
I need a payment method manager component
```
...
```bash
Include a usage meter component for my billing dashboard
```
